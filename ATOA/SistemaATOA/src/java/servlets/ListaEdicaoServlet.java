 ...
 * @author JotaWind
 */
@WebServlet(name = "ListaEdicaoServlet", urlPatterns = {"/ListaEdicaoServlet"})
public class ListaEdicaoServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("listaEdicao".equals(action)) {
            try {
                List<EdicaoAtividade> edicoes = facade.listaEdicaoAtividade();
                request.setAttribute("edicoes", edicoes);
            } catch (ClassNotFoundException | SQLException ex) {
                status = "error";
            }
            RequestDispatcher rd = getServletContext().getRequestDispatcher("/atividades/aprovarCorrecao.jsp" );
            ...
            rd.forward(request, response);
        } else if ("recusada".equals(action)) {
            try {
                String idEdicao = request.getParameter("id");
                facade.recusarEdicao(facade.getEdicaoPorID(Integer.parseInt(idEdicao)));
            } catch (ClassNotFoundException | SQLException ex) {
                status = "error";
            }
            RequestDispatcher rd = getServletContext().getRequestDispatcher("/ListaEdicaoServlet?action=listaEdicao" );
